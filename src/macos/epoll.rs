...
impl EpollEvent {
    ...
    pub fn data(&self) -> u64 {
        //println!("EpollEvent data: {}", self.u64);
        self.u64
    }
        ...
        match operation {
            ControlOperation::Add | ControlOperation::Modify => {
                if eset.contains(EventSet::IN) {
                    self.watcher
                        .add_fd(fd, EventFilter::EVFILT_READ, FilterFlag::empty(), event.data())?;
                }
                if eset.contains(EventSet::OUT) {
                    self.watcher
                        .add_fd(fd, EventFilter::EVFILT_WRITE, FilterFlag::empty(), event.data())?;
                }
            }
    ...
    pub fn wait(&self, timeout: i32, events: &mut [EpollEvent]) -> io::Result<usize> {
        ...
        match self.watcher.poll_forever(tout) {
            Some(event) => {
                ...
                match event.data {
                    EventData::ReadReady(data) => {
                        events[0].events = EventSet::IN.bits();
                        events[0].u64 = data as u64;
                    },
                    EventData::WriteReady(data) => {
                        events[0].events = EventSet::OUT.bits();
                        events[0].u64 = data as u64;
                    },
